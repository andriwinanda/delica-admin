        ...
        <b-table :data="listData" :loading="isLoading" :hoverable="true" :sticky-header="true">
          ...
          <template slot-scope="props">
            ...
            <b-table-column label="Series Tersedia" class="is-capitalized">
              <span v-for="(item, idx) in props.row.availability" :key="item">{{ item }}{{ idx < props.row.availability.length - 1 ? ', ': '' }}</span>
            </b-table-column>
            <b-table-column  label="Jumlah Daun Tersedia" class="is-capitalized">
              <span v-for="(item, idx) in props.row.doorLeaves" :key="item">{{ item }} Daun{{ idx < props.row.doorLeaves.length - 1 ? ', ': '' }}</span>
            </b-table-column>
            <b-table-column  label="Action" centered style="min-width: 80px">
              <div class="buttons has-addons is-centered">
                <b-button
                  ...
                  size="is-small"
                  icon-left="pencil"
                  @click="$router.push(`/series/edit/${props.row.idSeries}`)"
                ></b-button>
                <b-button
                  type="is-danger"
                  size="is-small"
                  icon-left="delete"
                  @click="deleteItem(props.row.idSeries)"
                ></b-button>
              </div>
            </b-table-column>
          </template>
